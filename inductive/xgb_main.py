...
# Define parser
parser = argparse.ArgumentParser(description='XGBoost Evaluation Script')
...
parser.add_argument('--learning_rate', type=float, default=0.001, help='Learning rate for boosting')
...
def xgb_evaluation(clf, max_num_epochs=200, batch_size=128, num_repetitions=10):
...
patient_dict = xgb_evaluation(clf, max_num_epochs=args.max_num_epochs, batch_size=args.batch_size, num_repetitions=args.num_repetitions)

# Compute metrics for each patient
average_ratio_per_label = []
percentage_corrected_labels = []
results_dir = "res_ind_xgb"
if not os.path.exists(results_dir):
    os.makedirs(results_dir)
results_file = os.path.join(results_dir, "res_ind_xgb.txt")
